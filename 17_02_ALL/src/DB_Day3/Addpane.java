...
/*
 * 사용자로부터 입력을 받거나 처리된 결과를 보여주기 위한 어플리케이션의 프레젠테이션 기능을 구현
 * 사용자가 쉽게 사용할 수 있도록 UI 컴포넌트를 배열하고 네비게이션 정책을 수립
 * 
 * AddPane() : 사원정보 입력 탭 객체는 정보를 입력 받기 위한 텍스트필드 배열, 저장하기 버튼
 * 다시쓰기버튼으로 구성할 것임. 
 * 
 * 
 */
public class Addpane extends JPanel implements ActionListener, ItemListener {
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(ae_type.equals(okb.getText())){
			...
			if(edvo!=null) JOptionPane.showMessageDialog(this, tf[0].getText()+"님이 성공적으로 추가됨");
			
		
		}else if(ae_type.equals(rsb.getText())){
			int size = caption.length;
			for(int i=0; i< size-1;i++){
				tf[i].setText("");		//초기화 먼저 해준다. 
			}
		}
